...
int main() {
    ...
    int work_groups=2048, source_start, source_end;
    ...
    #pragma omp parallel for private(source_start, source_end)
    for(int i=0; i<work_groups; i++) {
        if(i==work_groups-1) {
            source_start=(TAMANHO/work_groups)*i;
            source_end=TAMANHO;
            tokens[i]=get_tokens(source, source_start, source_end);
        } else {
            source_start=(TAMANHO/work_groups)*i;
            source_end=(TAMANHO/work_groups)*(i+1);
            tokens[i]=get_tokens(source, source_start, source_end);
        }
    }
